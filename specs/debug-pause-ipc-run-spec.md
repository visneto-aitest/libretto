...
- [x] In `packages/libretto/src/debug/pause.ts`, remove `.paused/.resume` file IO and wait loop behavior.
- [x] Introduce a typed pause signal (for example, `DebugPauseSignal` + guard helper) that includes at least `url` and `pausedAt`.
- [x] Make `debugPause` behavior:
  - return immediately when disabled,
  - log pause context when enabled,
  - throw the typed pause signal when enabled.
- [x] Ensure exports in `packages/libretto/src/debug/index.ts` and package root remain coherent for new pause signal types/helpers.
- [x] Success criteria: add/adjust unit tests to verify enabled `debugPause` throws typed signal and disabled `debugPause` is a no-op, with no file artifacts created.
