...
Run the probes from `integration-approach-selection.md` (in the `libretto` skill's directory: `.agents/skills/libretto/integration-approach-selection.md`). This answers one question: **which approaches are safe to use on this site?**
...
When codifying UI interaction steps, **translate .bin/libretto-cli `exec` code into proper Playwright locator APIs**. Do not copy-paste `page.evaluate()` / `document.querySelector()` patterns from the interactive session into the production file.
