...
This installs Libretto, sets up the `.libretto/` directory, copies agent skill files, and walks you through configuring an AI provider for snapshot analysis. If Chromium isn't already installed, it downloads it automatically. See [Configuration](./configuration) for details on providers and config options.
