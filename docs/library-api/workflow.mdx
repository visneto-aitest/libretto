...
export const main = workflow<{ query: string }, string[]>(
  "main",
  async (ctx, input) => {
    const { page } = ctx;
...
Pass the file path and the workflow's **runtime name** to `libretto run`. The runtime name is the `name` string you pass as the first argument to `workflow(name, handler)`, not the JavaScript export identifier:
...
<Note>
  The CLI resolves workflows by their runtime `name` string, not by the JavaScript variable or export name. If you write `export const myExport = workflow("main", handler)`, the CLI argument must be `main`, not `myExport`.

**Module discovery:** The CLI scans all exports from your file. It supports direct named exports (e.g. `export const foo = workflow(...)`), a special `workflows` export that is either a single workflow or an object map of workflows (e.g. `export const workflows = { foo, bar }`), and resolves the target workflow by matching the runtime `name` string across all discovered workflows.
...
export const main = workflow<{ id: string }, void>(
  "main",
  async (ctx, input) => {
